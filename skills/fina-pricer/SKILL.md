...
2. Normalize dates, currencies, calendars, notionals, underlyings, strikes, barriers, observation dates, and applied fixings. Reject missing expiry, inconsistent valuation dates, non-positive prices, and ambiguous RFKs. Support one to three underlyings only; basket pricing currently requires a common strike and uses `basket_method="worst_of"` by default, with optional positive-definite correlation input. Each underlying may define its own `barriers` list; these are evaluated on that underlying’s normalized path. Top-level `parameters.barriers` remains a shared basket barrier for backward compatibility.
