...
6. Generate the full Greek vector for every requested RFK: `delta`, `gamma`, `vega`, `rho`, `theta`, `fx_delta`, `vanna`, `volga`, and `charm`. Preserve zero values when a Greek is structurally irrelevant, and explain why. Include method, backend, bump details if applicable, tape/path counts, and error estimates.
7. Construct the RiskCube using stable axes `Underlying`, `Expiry`, `Strike`, and `RiskFactorType`. Keep one cell per RFK and include explainability diagnostics: moneyness, time to expiry, barrier events, lifecycle state, applied fixings, coupon-memory carry, and model/backend.
8. Validate financial sanity. Check PV non-negativity where applicable, finite PV and Greeks, put/call direction, barrier state transitions, coupon-memory release, FX scaling, and convergence across seeds or path counts. Compare AAD against finite differences only as a validation diagnostic, never as the production method when AAD is required.
