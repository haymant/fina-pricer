...
|---|---|---|
| `version_catalog` | Persists release/batch versions and allocates compact surrogate IDs | `version_id`, `version_key`, `version_name`, `created_at` |
| `scenario_catalog` | Persists materialized snapshots or rule-based scenario definitions | `scenario_id`, `scenario_key`, `scenario_version`, `scenario_kind`, `materialization_mode` |
| `riskcube_instances` | Coordinates a batch/version/scenario execution | `instance_id`, `batch_id`, `version_id`, `scenario_id`, `status` |
| `riskcube_partitions` | Maps a completed instance to its Parquet partition | `partition_id`, `instance_id`, `version_id`, `scenario_id`, `parquet_path` |
...
`version_catalog` and `scenario_catalog` are durable metadata tables. Each stable `version_key` and `scenario_key` receives an auto-incrementing integer `version_id` or `scenario_id`. The integer IDs are used in joins, filters, and object paths; the stable keys remain available for governance and human-readable audit trails. `riskcube_instances.scenario_id` and `riskcube_instances.version_id` identify the catalog records, while `riskcube_partitions.instance_id` links persisted cells back to the execution instance.
...
```text
<root>/version_id=<version_id>/scenario_id=<scenario_id>/instance_id=<instance_id>.parquet
```
...
FROM read_parquet('s3://fina-riskcube/version_id=*/scenario_id=*/*.parquet')
WHERE version_id = 1
...
The batch executor persists or reuses a scenario definition, persists or reuses the version record, creates a `riskcube_instances` row with `RUNNING` status, materializes each request, calls the pricing engine, inserts flattened cells into DuckDB, writes one immutable Parquet partition under integer partition keys, and marks the instance `COMPLETED`. The same design can later be extended with a queue or parallel worker pool without changing the storage contract.
