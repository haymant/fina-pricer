...
2. Normalize dates, currencies, calendars, notionals, underlyings, strikes, barriers, observation dates, and applied fixings. Reject missing expiry, inconsistent valuation dates, non-positive prices, and ambiguous RFKs. Support one to three underlyings only, with an independent `strikePrice` for each underlying. Each underlying may define its own `barriers` list; these are evaluated on that underlying’s normalized path. Top-level `parameters.barriers` remains a shared basket barrier for backward compatibility.
   Leg-specific lifecycle is authoritative when present: define `Legs[].ki_enabled`, `Legs[].already_knock_in`, `Legs[].ki_monitoring`, `Legs[].global_ko`, `Legs[].barriers`, and `Legs[].schedule` separately for the intrinsic/put and coupon legs. `ki_monitoring="AKI"` monitors the put barrier across the path; `ki_monitoring="EKI"` tests only the final fixing date or the last `schedule.fixing_dates` date. When `Legs[].global_ko.enabled=true` and `locked=true`, the put leg is immediately terminated at `effective_date` (or `date`) and contributes zero thereafter. Coupon KI is **disabled by default** and must not inherit the product-level lifecycle flag; enable it explicitly on the coupon leg. `Legs[].observation_dates` and `payment_dates` (or the nested schedule) override the aggregate accrual schedule. A put may use `strike_ratio` with each underlying’s `reference_price` and `settlement="delivery"` when the term sheet defines redemption as a strike-ratio delivery payoff.
