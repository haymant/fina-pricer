...
2. Normalize dates, currencies, calendars, notionals, underlyings, strikes, barriers, observation dates, and applied fixings. Reject missing expiry, inconsistent valuation dates, non-positive prices, and ambiguous RFKs. Support one to three underlyings only, with an independent `strikePrice` for each underlying. Each underlying may define its own `barriers` list; these are evaluated on that underlying’s normalized path. Top-level `parameters.barriers` remains a shared basket barrier for backward compatibility.
   Leg-specific lifecycle is authoritative when present: define `Legs[].ki_enabled`, `Legs[].already_knock_in`, `Legs[].ki_monitoring`, `Legs[].option_ko`, `Legs[].barriers`, and `Legs[].schedule` separately for the intrinsic/put leg. `option_ko` maps `globalKOStar`: its `global_ko` flag controls option termination, while `observation_dates` and `effective_dates` represent the KO and interest-rate effective schedules. Terminate the put only when `option_ko.enabled=true`, `option_ko.global_ko=true`, and an effective date is on or before evaluation. Do not map `GKOLocked/GKODate` into option termination. Those fields belong in `Legs[].memory_ko`, which models coupon-memory lifecycle state with `locked`, `dates`, `performances`, and `memory_action`. `ki_monitoring="AKI"` monitors the put barrier across the path; `ki_monitoring="EKI"` tests only the final fixing date or the last `schedule.fixing_dates` date. Coupon KI is **disabled by default** and must not inherit the product-level lifecycle flag; enable it explicitly on the coupon leg. `Legs[].observation_dates` and `payment_dates` (or the nested schedule) override the aggregate accrual schedule. A put may use `strike_ratio` with each underlying’s `reference_price` and `settlement="delivery"` when the term sheet defines redemption as a strike-ratio delivery payoff.
3. Build market handles. Map each underlying’s spot RFK to its own market value, vol RFK to the calibrated volatility surface, interest-rate RFK to the discount curve, and FX RFK to FX spot handles. Keep each RFK’s identity attached to the variable. Basket paths use correlated GBM and report one RiskCube cell per requested underlying RFK. When `MarketDataSnapshot.vol_surfaces` is supplied, interpolate bilinearly by strike and maturity at each underlying’s actual `strikePrice` and product expiry; use single-point `vol_data` only as fallback. Preserve the source maturity convention, including numeric date serials or ISO dates, and reject malformed or non-positive grids. Set `parameters.volatility_mode="forward"` to convert surface total variance into piecewise forward volatilities over the simulation steps; the default `constant` mode uses the interpolated terminal volatility throughout.
...
When an instrument is already knocked in, set `UpdatedLifecycle.already_knock_in=true`. The intrinsic downside option leg must then be active from valuation onward even when no new KI barrier is included in `parameters.barriers`; an explicit short put leg must be reported with a negative PV. Do not double-apply the short sign to an already signed FCN aggregate payoff.
