...
2. Normalize dates, currencies, calendars, notionals, underlyings, strikes, barriers, observation dates, and applied fixings. Reject missing expiry, inconsistent valuation dates, non-positive prices, and ambiguous RFKs. Support one to three underlyings only. The current worst-of/best-of payoff implementation requires a common strike for basket redemption normalization; this is independent of barrier configuration and does not require common barriers. Each underlying may define its own `barriers` list; these are evaluated on that underlying’s normalized path. Top-level `parameters.barriers` remains a shared basket barrier for backward compatibility.
