...
For range-accrual coupons, accept `n1` and `n2` arrays with one entry per coupon period. Treat past `n1` values as fixed realized accruing-day counts and future `n1` values as pathwise stochastic counts. For future periods, count simulated daily/step observations inside `range_lower`/`range_upper`, then calculate the period coupon as `period_coupon_rate × N1_path / N2`. Apply memory to the period shortfall and discount the expected future coupon cashflows. Record `fixed_n1_periods`, the range convention, schedule arrays, and whether future `N1` was stochastic in explainability diagnostics. Reject mismatched `n1`/`n2` lengths and do not assume `N1=N2` for unfixed future periods.
